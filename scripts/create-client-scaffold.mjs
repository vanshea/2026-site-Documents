...
const CONTENT_CLIENTS_ROOT = path.join(DOCUMENTS_ROOT, "content", "clients");
...
function buildStarterContent({ clientId, title, access }) {
  return {
    ...
    title,
    access,
    coverImage: `${ANALYTICS_BASE_PATH}/clients/${clientId}/cover/cover.svg`,
...
async function main() {
  ...
  const contentPath = path.join(CONTENT_CLIENTS_ROOT, `${options.clientId}.json`);
