...
function normalizeVideo(video, fallbackTitle) {
  ...
  return {
    ...
    title: String(video?.title || fallbackTitle).trim()
  };
}

function normalizePresentation(presentation, fallbackTitle) {
  const src = String(presentation?.src || "").trim();
  if (!src) return null;

  return {
    src,
    title: String(presentation?.title || fallbackTitle).trim()
  };
}
...
function normalizeCaseStudy(rawCaseStudy, fallbackSlug) {
  ...
  const heroPresentation = normalizePresentation(
    rawCaseStudy?.heroPresentation,
    `${title} presentation`
  );
  const heroVideo = normalizeVideo(rawCaseStudy?.heroVideo, `${title} video`);
